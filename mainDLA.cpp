...
// functions which are needed for openGL go into a namespace so that we can
// identify them
namespace drawFuncs {
...
void drawFuncs::introMessage() {
  std::cout << "Keys (while in graphics window):" << std::endl
            << "  q or e to quit (or exit)" << std::endl;
  ...
  std::cout << "  w or b to change background colour to white or black"
            << std::endl;
}
...
void drawFuncs::display() {
  ...
  // this puts the camera at the origin (not sure why) with (I think) z axis out
  // of page and y axis up there is also the question of the GL perspective
  // which is not set up in any clear way at the moment
