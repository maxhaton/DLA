...
void DLASystem::moveLastParticle() {
	...
	// check if destination is empty
		...
		setGrid(lastP.pos.data(), 0);  // set the old grid site to empty
		// update the position
		particleList[numParticles - 1].pos[0] = newpos[0];
		particleList[numParticles - 1].pos[1] = newpos[1];
		setGrid(lastP.pos.data(), 1);  // set the new grid site to be occupied

		// check if we stick
		if (checkStick()) {
			//cout << "stick" << endl;
			setParticleInactive();  // make the particle inactive (stuck)
			updateClusterRadius(lastP.pos.data());  // update the cluster radius, addCircle, etc.
...
int DLASystem::checkStick() {
	...
	for (int i = 0; i < 4; i++) {
		double checkpos[2];
		setPosNeighbour(checkpos, lastP.pos.data(), i);
...
void DLASystem::DrawSquares() {
	...
	for (int p = 0; p < numParticles; p++) {
		double *vec = particleList[p].pos.data();
		glPushMatrix();
