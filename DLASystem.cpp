...
// but this corresponds to the middle of the grid array ie grid[ halfGrid ][
// halfGrid ]
void DLASystem::setGrid(std::array<double, 2> pos, bool val) {
  assert(val == 0 || val == 1);
  int halfGrid = gridSize / 2;
  int x = static_cast<int>(pos[0] + halfGrid);
  int y = static_cast<int>(pos[1] + halfGrid);
  unsigned int offset = this->gridSize * y + x;
  grid[offset] = val;
}
...
// to be safe, we need the killCircle to be at least 2 less than the edge of the
// grid
...
void DLASystem::addParticle(std::array<double, 2> pos) {
  ...
  particleList.push_back(Particle{pos});
  numParticles++;
...
void DLASystem::addParticleOnAddCircle() {
  ...
  if (readGrid(pos) == 0)
    ...
    std::cout << "FAIL " << pos[0] << " " << pos[1] << std::endl;
}

// send back the position of a neighbour of a given grid cell
// NOTE: there is no check that the neighbour is inside the grid,
// this has to be done separately...
void DLASystem::setPosNeighbour(std::array<double, 2> &setpos,
...
// if the view is smaller than the kill circle then increase the view area (zoom
// out)
...
void DLASystem::updateClusterRadius(const std::array<double, 2> pos) {
...
void DLASystem::moveLastParticle() {
  int rr = rgen.randomInt(
      4); // pick a random number in the range 0-3, which direction do we hop?
  std::array<double, 2> newpos;

  Particle &lastP = particleList.at(numParticles - 1);
  ...
  // check if destination is empty
    ...
    if (checkStick()) {
      ...
      updateClusterRadius(
          lastP.pos); // update the cluster radius, addCircle, etc.

      if (numParticles % 100 == 0 && logfile.is_open()) {
        logfile << numParticles << " " << clusterRadius << std::endl;
      }
    }
  } else {
    ...
    std::cout << "reject " << rr << std::endl;
    std::cout << lastP.pos[0] << " " << lastP.pos[1] << std::endl;
...
DLASystem::DLASystem(Window *set_win) {
  std::cout << "creating system, gridSize " << gridSize << std::endl;
  win = set_win;
  numParticles = 0;
  endNum = 10000;

  grid = std::vector<bool>(this->gridSize * this->gridSize);
  slowNotFast = 1;
  // reset initial parameters
  Reset();

  addRatio = 1.2; // how much bigger the addCircle should be, compared to
                  // cluster radius
  killRatio =
      1.7; // how much bigger is the killCircle, compared to the addCircle
...
void DLASystem::DrawSquares() {
  ...
  for (int p = 0; p < numParticles; p++) {
    const auto vec = particleList[p].pos;
    glPushMatrix();
    ...
    glRectd(drawScale * (vec[0] - halfSize), drawScale * (vec[1] - halfSize),
            drawScale * (vec[0] + halfSize), drawScale * (vec[1] + halfSize));
    glPopMatrix();
  }

  // print some information (at top left)
  // this ostringstream is a way to create a string with numbers and words
  // (similar to cout << ... )
