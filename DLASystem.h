...
private:
  ...
  // int **grid;  // this will be a 2d array that stores whether each site is
  // occupied
  ...
  double addRatio; // how much bigger the addCircle should be, compared to
                   // cluster radius
  double
      killRatio; // how much bigger is the killCircle, compared to the addCircle
...
public:
  ...
  // lastParticleIsActive is +1 if there is an active particle in the system,
  // otherwise 0
  ...
  // check whether we should stop (eg the cluster has reached the edge of the
  // grid)
  int checkStop();
  ...
  // if pos is outside the cluster radius then set clusterRadius to be the
  // distance to pos.
  void updateClusterRadius(std::array<double, 2> pos);

  // set and read grid entries associated with a given position
  void setGrid(const std::array<double, 2> pos, const bool val);
  int readGrid(const std::array<double, 2> pos);

  // return the distance of a given point from the origin
  inline double distanceFromOrigin(double pos[]) {
    return sqrt(pos[0] * pos[0] + pos[1] * pos[1]);
  }
  inline double distanceFromOrigin(std::array<double, 2> pos) {
    return sqrt(pos[0] * pos[0] + pos[1] * pos[1]);
  }
  ...
  void addParticle(const std::array<double, 2> pos);
  ...
  void setPosNeighbour(std::array<double, 2> &setpos,
                       const std::array<double, 2> pos, const int val);
  ...
  // it would be better for an OOP philosophy to make these member functions for
  // the Window class but we are being a bit lazy here
